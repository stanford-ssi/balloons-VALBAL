...
DataFrame Simulator::readData() {
  ...
  simulated.GPS_GOOD_STATE = atof(values[41]);
  simulated.PRESS_BASELINE = atof(values[42]);
  simulated.INCENTIVE_NOISE = atof(values[43]);
  simulated.INCENTIVE_THRESHOLD = atof(values[44]);
  simulated.RE_ARM_CONSTANT = atof(values[45]);
  simulated.BALLAST_ARM_ALT = atof(values[46]);
  simulated.VALVE_SETPOINT = atof(values[47]);
  simulated.VALVE_DURATION = atof(values[48]);
  simulated.VALVE_ALT_LAST = atof(values[49]);
  simulated.VALVE_VELOCITY_CONSTANT = atof(values[50]);
  simulated.VALVE_ALTITUDE_DIFF_CONSTANT = atof(values[51]);
  simulated.VALVE_LAST_ACTION_CONSTANT = atof(values[52]);
  simulated.BALLAST_SETPOINT = atof(values[53]);
  simulated.BALLAST_DURATION = atof(values[54]);
  simulated.BALLAST_ALT_LAST = atof(values[55]);
  simulated.BALLAST_VELOCITY_CONSTANT = atof(values[56]);
  simulated.BALLAST_ALTITUDE_DIFF_CONSTANT = atof(values[57]);
  simulated.BALLAST_LAST_ACTION_CONSTANT = atof(values[58]);
  simulated.SHOULD_CUTDOWN = atof(values[59]);
  simulated.SHOULD_LED = atof(values[60]);
  simulated.SETUP_STATE = atof(values[61]);
  simulated.DEBUG_STATE = atof(values[62]);
  simulated.FORCE_VALVE = atof(values[63]);
  simulated.FORCE_BALLAST = atof(values[64]);
  simulated.REPORT_MODE = atof(values[65]);
  simulated.BMP_1_ENABLE = atof(values[66]);
  simulated.BMP_2_ENABLE = atof(values[67]);
  simulated.BMP_3_ENABLE = atof(values[68]);
  simulated.BMP_4_ENABLE = atof(values[69]);
  simulated.BMP_1_REJECTIONS = atof(values[70]);
  simulated.BMP_2_REJECTIONS = atof(values[71]);
  simulated.BMP_3_REJECTIONS = atof(values[72]);
  simulated.BMP_4_REJECTIONS = atof(values[73]);
