...
\usepackage[top=2.6in,left=0.62in,right=0.62in,bottom=0.62in]{geometry} 
...
\item Iridium, 8c/50 byte.
\item Custom 433~MHz radio, ~5~kBps line-of-sight.
\end{minipage}\vspace{1mm} }\\\midrule
\textbf{Cost\footnotemark} & \specialcell[t]{\emph{Conumables:} \$400\\\emph{Reusables:} \$1000}
...
\\\bottomrule
\end{tabular}
\footnotetext{\sffamily An endurance of five days was demonstrated with zero payload. This reflects our current best mission, but we believe that longer missions are feasible with the system.}
\footnotetext{\sffamily Reusable components can be reflown for many flights if the system is recovered.}
\end{center}
{\footnotesize 
\vspace{-.4cm}
\begin{center}
\noindent\includegraphics[width=0.8\linewidth,trim={2.9cm 1cm 2cm 1.5cm},clip]{allflights.png}
\end{center}
\vspace{-.8cm}
\paragraph{\sffamily Flight Heritage:} The sysyem has been developed though more than 20 test flights, 10 of which are shown above\vspace{-.4cm} 
...
\paragraph{\sffamily Ease of assembly: } ValBal requires only six custom components and can be assembled in under 2 hours. No manual machining, cutting, gluing, or soldering is required\footnote{above image shows a slightly older version than the current design}.
