    ...
    def watch_queue(self):
        ...
        while(True):
            ...
            if code == 1: # change garbage function
                ...
                # wait for conection from teensy until you register any listeners
                while not self.has_teensy:
                    time.sleep(0.1)
                time.sleep(0.1) # wait for listener to be ready
        ...
        while(True):
            ...
            if flag == 0x0:
                ...
                if msg != FSTART or self.first_write: # NOT (msg = FSTART and not self.first_write)
                    ...
                    if DEBUG: print("serialpublisher.py write: didn't write to teensy. Already hooked into serial")
                ...
                if DEBUG: print("serialpublisher.py write: de-registering new subscriber")
                read_write_queue.put((3, msg))
            elif flag == 0x4:
                if DEBUG: print("serialpublisher.py write: flushing serial buffer")
                teensy.flush()
...
if __name__=='__main__':
    setup()
    print("\n(Change directory and run shitl and/or serial monitor from here\n>>> ")
    read_serial()
    write_serial()
